# A simple library for ALL OAuth APIs #

Are you also growing tired of implementing different libraries to communicatie with the Twitter API, the Google API, the Facebook API etc.

It turns out that OAuth is actually really simple (thanks to Eris for pointing that out: http://erisds.co.uk/code/twitter-oauth-simple-curl-requests-for-your-own-data). This project contains one Oauth class with various child classes. The child classes (Google, Facebook etc) inherit from the Oauth class and in most cases can re-use almost all the functions from their parent.
...
The Facebook class looks like this:
```
<?php
...
And finally the Oauth class, the place where all the magic happens:
```
<?php
...
The code for the Google, Twitter and Linkedin classes can be found in the includes directory in the repository http://code.google.com/p/simple-php-oauth/source/browse/#svn%2Ftrunk%2Fincludes.

I have this working now with Google, Facebook, Twitter and Linkedin. More will follow soon :)
